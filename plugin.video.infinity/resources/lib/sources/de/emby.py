...
class source:
    def __init__(self):
        ...
        self.header_preauth={'Content-Type': 'application/json','Accept-Charset': 'UTF-8,*', 'X-Emby-Authorization': 'MediaBrowser Client="Kodi Infinity",Device="LAssthi",DeviceId="xxx",Version="1.0"', 'Accept-encoding': 'gzip', 'Authorization': 'MediaBrowser Client="Kodi Lasthip",Device="Infinity",DeviceId="xxx",Version="1.0.0"'}
        ...
        try:
            ...
            for itemid in itemid_liste:
                ...
                try:
                    ...
                    if '/vodhd/media/4K/' in resp['Items'][0]['MediaSources'][0]['Path']:
                        return_url['4K'] = self.server + "/emby/Videos/" + str(
                            resp['Items'][0]['Id']) + "/stream?static=true&PlaySessionId=1LIEC3&MediaSourceId=" + str(
                            resp['Items'][0]['MediaSources'][0]['Id']) + "&api_key=" + self.token
                    elif '/vodhd/media/Filme/' in resp['Items'][0]['MediaSources'][0]['Path']:
                        return_url['1080p'] = self.server + "/emby/Videos/" + str(
                            resp['Items'][0]['Id']) + "/stream?static=true&PlaySessionId=1LIEC3&MediaSourceId=" + str(
                            resp['Items'][0]['MediaSources'][0]['Id']) + "&api_key=" + self.token
    ...
    def __authenticate(self):
        try:
            ...
            self.header_postauth={'Content-Type': 'application/x-www-form-urlencoded','Accept-Charset': 'UTF-8,*', 'X-Emby-Authorization': 'MediaBrowser UserId='+self.userid+',Client="Kodi Infinity",Device="Infinity",DeviceId="xxx",Version="1.0"', 'Accept-encoding': 'gzip', 'Authorization': 'MediaBrowser UserId='+self.userid+',Client="Kodi Infinity",Device="Infinity",DeviceId="xxx",Version="1.0"', 'X-MediaBrowser-Token':self.token}
