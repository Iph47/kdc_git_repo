...
def get_release_quality(release_name, release_link=None):
    ...
    try:
        ...
        elif 'bdrip' in fmt: quality = '720p'
        ...
        fileType = getFileType(release_name)
        if not fileType == '': info.append(fileType)
...
def getFileType(url):
    ...
    if '.dts' in url: type += ' DTS 5.1/'
    if '5.1' in url: type += ' 5.1 /'
    if '.dl' in url: type += ' Direct Line /'
    if '.mic' in url: type += ' Microphone Dubbed /'
