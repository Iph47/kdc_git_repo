...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.infinity.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.infinity.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.infinity.artwork)') else setting('appearance.alt')
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.infinity.artwork)')
